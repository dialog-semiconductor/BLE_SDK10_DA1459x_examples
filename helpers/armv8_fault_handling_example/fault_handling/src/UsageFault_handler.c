...
extern volatile Fault_t _fault_exception_info __UNUSED;
        ...
        __UNUSED volatile uint32_t dbg_stacklimit_context;
#endif

/*
 * User-defined UsageFault handler. This is the 2nd part of the handler written in
...
void UsageFault_HandlerC(uint32_t *fault_args, uint32_t fp_context, uint32_t stacklimit_context)
{
        ...
        *MTB_MASTER_REG = MTB_MASTER_REG_DISABLE_VAL;
        ...
        hw_watchdog_freeze();
                ...
                dbg_stacklimit_context = stacklimit_context;

                /* UsageFault Status Register (UFSR) */
                __UNUSED volatile uint16_t usage_fault_status = _FLD2VAL(SCB_CFSR_USGFAULTSR, SCB->CFSR);
                 ...
                 *
                 * if stacklimit_context == 1:
                 *      - stack overflow took place on MSP
                 * else if stacklimit_context == 2:
                 *      - stack overflow took place on PSP
                 * else:
                 *      - no stack overflow
                 */
                ...
                _fault_exception_info.stacklimit_context = stacklimit_context;
