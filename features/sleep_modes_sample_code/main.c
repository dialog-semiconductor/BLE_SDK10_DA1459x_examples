 ...
 * @file main.c
 *
 * @brief FreeRTOS based application demonstrating device sleep modes.
 *
...
static OS_TASK_FUNCTION(system_init, pvParameters)
{
        ...
        pm_sleep_mode_set(APP_DEFAULT_SLEEP_MODE);

        /* Start ExportSystemClock task  */
        status = OS_TASK_CREATE("SleepModes",               /* The text name assigned to the task, for
                                                               debug only; not used by the kernel. */
                        prvSleepModesTask,                  /* The function that implements the task. */
