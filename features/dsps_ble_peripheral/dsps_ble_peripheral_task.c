 ...
 *
 * @brief DSPS BLE peripheral demo demonstrates RENESAS Serial Port Service.
...
static gap_adv_ad_struct_t adv_data[] = {
#if dg_configSUOTA_SUPPORT
# define UUID_SUOTA_LSB     (dg_configBLE_UUID_SUOTA_SERVICE & 0xFF)
# define UUID_SUOTA_MSB     ((dg_configBLE_UUID_SUOTA_SERVICE & 0xFF00) >> 8)

        GAP_ADV_AD_STRUCT_BYTES(GAP_DATA_TYPE_UUID128_LIST, 0),
        GAP_ADV_AD_STRUCT_BYTES(GAP_DATA_TYPE_UUID16_LIST_INC, UUID_SUOTA_LSB, UUID_SUOTA_MSB)
...
static const gap_adv_ad_struct_t scan_rsp[] = {
...
static void throughput_calculation(uint32_t dataSize, SPS_DIRECTION direction)
{
        ...
        static uint32_t former_ticks[SPS_DIRECTION_MAX] = { 0 };
        static uint32_t accumulated_size[SPS_DIRECTION_MAX] = { 0 };
...
static void tx_done_cb(ble_service_t *svc, uint16_t conn_idx)
{
        ...
        if(send_flow_on) {
                ...
                SERIAL_PORT_SET_FLOW_ON(UART_DSPS_DEVICE);
...
#if defined(DSPS_UART)
        uart_handle = SERIAL_PORT_OPEN(UART_DSPS_DEVICE);
...
#if defined(DSPS_UART)
  #if defined(CFG_UART_HW_FLOW_CTRL)
        SERIAL_PORT_SET_FLOW_OFF(UART_DSPS_DEVICE);
  #elif defined(CFG_UART_SW_FLOW_CTRL)
...
OS_TASK_FUNCTION(dsps_peripheral_task, pvParameters)
{
        ...
        dsps = (dsps_service_t *)dsps_init(&sps_callbacks);
        ...
        adv_data[0].len = sizeof(sps_uuid.uuid128);
        adv_data[0].data = sps_uuid.uuid128;

        ble_gap_adv_ad_struct_set(ARRAY_LENGTH(adv_data), adv_data, ARRAY_LENGTH(scan_rsp), scan_rsp);
        ble_gap_adv_intv_set(BLE_ADV_INTERVAL_FROM_MS(20), BLE_ADV_INTERVAL_FROM_MS(30));
